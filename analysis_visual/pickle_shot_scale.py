...
OUTPUT_FILENAMES = {
    "square": "videoshot_scale.pkl",
    "centercrop": "videoshot_scale_centercrop.pkl",
    "fivecrop": "videoshot_scale_fivecrop.pkl",
}
...
def parse_args():
    parser = argparse.ArgumentParser(description="Predict Shot Scale using a custom VideoMAE model.")
...
def get_model(device):
    path = "gullalc/videomae-base-finetuned-kinetics-movieshots-scale"
...
def predict_shot(inputs, model, crop_type, processor, device):
    ...
    scale_cat = model.config.id2label[scale_pred]

    return scale_cat, cumulative_distance


def process_video(video_path, shot_dict, model, transform, processor, crop_type, args, device="cuda"):
    """
    Take VideoMAE model and predicts shot scale for each shot in the video.

    Args:
        video_path (str): path to video
        shot_dict (dict): loaded shot detection output
        model (VideoMAEForVideoClassification): VideoMAE model
    ...
    Returns:
        video_feat_dict (dict): dictionary containing predicted shot scale for each shot in the video.
        As:
        {
            "github_repo": "https://huggingface.co/gullalc/videomae-base-finetuned-kinetics-movieshots-scale",
            ...
            "output_data": [
                {
                    ...
                    "prediction": [predicted_scale_class, cummulative_distance]
                }
            ]
        }
    """

    video_feat_dict = {"github_repo": "https://huggingface.co/gullalc/videomae-base-finetuned-kinetics-movieshots-scale",
                        "commit_id": "23e38eaf25a909c8f5d00c6803172e50cf2058ee",
                        "parameters": "default",
                        "video_file": video_path,
                        "output_data": []
    ...
    for i, ref_shot in enumerate(tqdm(shot_dict["output_data"]["shots"])):
        ...
        scale_cat, cumulative_distance = predict_shot(inputs, model, crop_type, processor, device)

        video_feat_dict["output_data"].append({"shot": {"start": ref_shot["start"], "end": ref_shot["end"]},
                                               "prediction": [scale_cat, cumulative_distance]})
